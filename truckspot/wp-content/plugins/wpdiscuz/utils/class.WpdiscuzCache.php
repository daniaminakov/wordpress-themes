...
class WpdiscuzCache implements WpDiscuzConstants {
	...
	public function purgeAllCaches() {
		if (current_user_can("manage_options") && isset($_GET["_wpnonce"]) && wp_verify_nonce($_GET["_wpnonce"], "purgeAllCaches")) {
	...
	public function purgePostCaches() {
		if (current_user_can("manage_options") && isset($_GET["_wpnonce"]) && !empty($_GET["post_id"]) && wp_verify_nonce($_GET["_wpnonce"], "purgePostCaches")) {
			$this->resetCommentsCache($_GET["post_id"]);
			$this->resetUsersCache();
		}
